...
set(ELTEN_WINDOWS_X64_MSYS2_PACKAGES "base-devel mingw-w64-ucrt-x86_64-gcc mingw-w64-ucrt-x86_64-make mingw-w64-ucrt-x86_64-pkgconf mingw-w64-ucrt-x86_64-sqlite3 mingw-w64-ucrt-x86_64-libxslt" CACHE STRING "MSYS2 packages installed for Windows x64 native gem builds")
set(ELTEN_WINDOWS_X86_MSYS2_PACKAGES "base-devel mingw-w64-i686-gcc mingw-w64-i686-make mingw-w64-i686-pkgconf mingw-w64-i686-sqlite3 mingw-w64-i686-libxslt" CACHE STRING "MSYS2 packages installed for Windows x86 native gem builds")
set(ELTEN_WINDOWS_ARM64_MSYS2_PACKAGES "base-devel mingw-w64-clang-aarch64-gcc mingw-w64-clang-aarch64-make mingw-w64-clang-aarch64-pkgconf mingw-w64-clang-aarch64-sqlite3 mingw-w64-clang-aarch64-libxslt" CACHE STRING "MSYS2 packages installed for Windows arm64 native gem builds")
...
function(elten_configure_ruby_runtime)
  ...
  if(ARG_PLATFORM STREQUAL "windows")
    if(ARG_ARCH STREQUAL "x64")
      ...
      set(msys2_packages "${ELTEN_WINDOWS_X64_MSYS2_PACKAGES}")
      set(nokogiri_msys2_package "mingw-w64-ucrt-x86_64-libxslt")
    elseif(ARG_ARCH STREQUAL "x86")
      ...
      set(msys2_packages "${ELTEN_WINDOWS_X86_MSYS2_PACKAGES}")
      set(nokogiri_msys2_package "mingw-w64-i686-libxslt")
    elseif(ARG_ARCH STREQUAL "arm64")
      set(installer_arch "arm")
      set(default_dll "aarch64-ucrt-ruby${runtime_dll_abi}.dll")
      set(msys2_packages "${ELTEN_WINDOWS_ARM64_MSYS2_PACKAGES}")
      set(nokogiri_msys2_package "mingw-w64-clang-aarch64-libxslt")
    else()
    ...
    list(APPEND script_args
      ...
      "-DNOKOGIRI_BUILD_OPTIONS=--use-system-libraries"
    )
    # CACHE values created before libxslt became required keep their old defaults.
    # Add the architecture-specific package locally while preserving user extras.
    separate_arguments(msys2_package_list NATIVE_COMMAND "${msys2_packages}")
    list(FIND msys2_package_list "${nokogiri_msys2_package}" nokogiri_msys2_package_index)
    if(nokogiri_msys2_package_index EQUAL -1)
      list(APPEND msys2_package_list "${nokogiri_msys2_package}")
    endif()
    list(JOIN msys2_package_list " " msys2_packages)
    list(APPEND script_args "-DMSYS2_PACKAGES=${msys2_packages}")
